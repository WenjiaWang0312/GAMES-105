...
def part1_simple(viewer, target_pos):
    ...
    meta_data = MetaData(joint_name, joint_parent, joint_initial_position,
                         'RootJoint', 'lWrist_end')#lWrist_end
...
def part1_hard(viewer, target_pos):
    ...
    joint_name, joint_parent, joint_initial_position = viewer.get_meta_data()
    meta_data = MetaData(joint_name, joint_parent, joint_initial_position,
                         'lToeJoint_end', 'lWrist_end')#lWrist_end
    joint_position = viewer.get_joint_positions()
    joint_orientation = viewer.get_joint_orientations()
    ...
    class UpdateHandle:
        ...
        def update_func(self, viewer):
            ...
            self.joint_position, self.joint_orientation = part1_inverse_kinematics(
                meta_data, self.joint_position, self.joint_orientation,
                target_pos)
    ...
    class UpdateHandle:
        ...
        def update_func(self, viewer):
            joint_position, joint_orientation = part2_forward_kinematics(
                ...
                self.motion_data, self.current_frame)
            joint_position, joint_orientation = part2_inverse_kinematics(
                self.meta_data, joint_position, joint_orientation, 0.1, 0.3,
                1.4)
...
def bonus(viewer, left_target_pos, right_target_pos):
    ...
    class UpdateHandle:
        def __init__(self, left_marker, right_marker, joint_position,
                     joint_orientation):
    ...
    handle = UpdateHandle(left_marker, right_marker, joint_position,
                          joint_orientation)
    handle.update_func(viewer)
...
def main():
    ...
    # part1_hard(viewer, np.array([0.5, 1, 1.5]))
